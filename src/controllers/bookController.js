...
const getXINRBooks=async function(req,res){
    let allBooks= await BookModel.find({"prices.indianPrice":{$in:["500INR","200INR","100INR"]}
})
    res.send({msg:allBooks})
...
const getRandomBooks = async function(req,res){
    let allBooks = await BookModel.find({$or :[{isStockAvailable:false},{totalpages:{$gt:500}}]})
    res.send({msg:allBooks})
  }
